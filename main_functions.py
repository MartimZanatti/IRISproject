import os
import rouge
from  doc_class import doc_class, get_text
...
def summarization(doc, model):

    #similarity_matrix, ids_dict = create_sim_matrix_word_2_vec(doc.paragraphs, model, bert=False)
    similarity_matrix, ids_dict = create_sim_matrix_bert(doc.paragraphs, model)
...
def process_sum_to_files(paragraphs, scores, ids_dict, file_name, num_paragraphs, path):
...
def rouge_main(data, real_sum_data, rouge_path):
    ...
    for file_name in files:
        print(file_name)
        doc = doc_class(data + file_name, False)
        doc = create_stanza_sentences(doc)
        stanza_text = stanza_to_text(doc.paragraphs)
        real_sum = doc_class(real_sum_data + file_name, False)
...
def get_box_plot_data(labels, bp, values):
    rows_list = []
    ...
    for i in range(len(labels)):
        ...
        dict1["media"] = sum(values[i])/len(values[i])
...
def box_plot_main(paths):
    ...
    ax.set_ylim([0, 1])
    ax.set_xticklabels(["word2vec", "mlm-sts", "tsdae-sts", "tsdae-gpl-nli-sts", "bertimbau", "tsdae", "tsdae-gpl"])
    ...
    ax.set_ylim([0, 1])
    ax.set_xticklabels(["word2vec", "mlm-sts", "tsdae-sts", "tsdae-gpl-nli-sts", "bertimbau", "tsdae", "tsdae-gpl"])
    all_r_rouge_1 = [np.array(rouge_1_r["0"]), np.array(rouge_1_r["1"]), np.array(rouge_1_r["2"]), np.array(rouge_1_r["3"]), np.array(rouge_1_r["4"]), np.array(rouge_1_r["5"]), np.array(rouge_1_r["6"])]
    bp_p = ax.boxplot(all_r_rouge_1, sym='')
    print("recall 1: " , get_box_plot_data(["word2vec", "mlm-sts", "tsdae-sts", "tsdae-gpl-nli-sts", "bertimbau", "tsdae", "tsdae-gpl"], bp_p, all_r_rouge_1))
    ...
    ax = fig.add_axes([0.15, 0.1, 0.7, 0.8])
    ax.set_ylim([0, 1])
    ax.set_xticklabels(["word2vec", "mlm-sts", "tsdae-sts", "tsdae-gpl-nli-sts", "bertimbau"])
    ...
    ax.set_ylim([0, 1])
    ax.set_xticklabels(["word2vec", "mlm-sts", "tsdae-sts", "tsdae-gpl-nli-sts", "bertimbau", "tsdae", "tsdae-gpl"])
    all_r_rouge_2 = [np.array(rouge_2_r["0"]), np.array(rouge_2_r["1"]), np.array(rouge_2_r["2"]), np.array(rouge_2_r["3"]), np.array(rouge_2_r["4"]), np.array(rouge_2_r["5"]), np.array(rouge_2_r["6"])]
    bp_p = ax.boxplot(all_r_rouge_2, sym='')
    ax.set_title("recall 2_gram. ")
    print("recall 2: ", get_box_plot_data(["word2vec", "mlm-sts", "tsdae-sts", "tsdae-gpl-nli-sts", "bertimbau", "tsdae", "tsdae-gpl"], bp_p, all_r_rouge_2 ))
    ...
    ax.set_ylim([0, 1])
    ax.set_xticklabels(["word2vec", "mlm-sts", "tsdae-sts", "tsdae-gpl-nli-sts", "bertimbau", "tsdae", "tsdae-gpl"])
    all_r_rouge_l = [np.array(rouge_l_r["0"]), np.array(rouge_l_r["1"]), np.array(rouge_l_r["2"]), np.array(rouge_l_r["3"]), np.array(rouge_l_r["4"]), np.array(rouge_l_r["5"]), np.array(rouge_l_r["6"])]
    bp_p = ax.boxplot(all_r_rouge_l, sym='')
    ax.set_title("recall longest common sequence.")
    print("recall L: ", get_box_plot_data(["word2vec", "mlm-sts", "tsdae-sts", "tsdae-gpl-nli-sts", "bertimbau", "tsdae", "tsdae-gpl"], bp_p, all_r_rouge_l))
...
def box_plot_main_2(paths):
    ...
    ax = fig.add_axes([0.15, 0.1, 0.7, 0.8])
    ax.set_ylim([0, 1])
    ax.set_xticklabels(["word2vec", "word2vec_no_preprocessing"])
    all_r_rouge_1 = [np.array(rouge_1_r["0"]), np.array(rouge_1_r["1"])]
    bp_p = ax.boxplot(all_r_rouge_1, sym='')
    print("recall 1: " , get_box_plot_data(["word2vec", "word2vec_no_preprocessing"], bp_p, all_r_rouge_1))
    ...
    ax.set_ylim([0, 1])
    ax.set_xticklabels(["word2vec", "word2vec_no_preprocessing"])
    all_r_rouge_2 = [np.array(rouge_2_r["0"]), np.array(rouge_2_r["1"])]
    bp_p = ax.boxplot(all_r_rouge_2, sym='')
    ax.set_title("recall 2_gram. ")
    print("recall 2: ", get_box_plot_data(["word2vec", "word2vec_no_preprocessing"], bp_p, all_r_rouge_2 ))
    plt.savefig("r_rouge_2.png")
    ...
    ax.set_ylim([0, 1])
    ax.set_xticklabels(["word2vec", "word2vec_no_preprocessing"])
    all_r_rouge_l = [np.array(rouge_l_r["0"]), np.array(rouge_l_r["1"])]
    bp_p = ax.boxplot(all_r_rouge_l, sym='')
    ax.set_title("recall longest common sequence.")
    print("recall L: ", get_box_plot_data(["word2vec", "word2vec_no_preprocessing"], bp_p, all_r_rouge_l))
    plt.savefig("r_rouge_l.png")
